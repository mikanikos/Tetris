 ...
 * collide con una {@link Cella} giÃ  presente. 
 * <BR/>
...
public class Pozzo {
	...
	public Pozzo(int l, int h) {
		this.celle = new TreeSet<>(new ComparatoreXY());/* DA COMPLETARE (esercizi 2,3,4) */
		this.addBordo(l, h);
	}
	...
	NavigableSet<Integer> getInsiemeOrdinateY(Set<Cella> inputCelle) {
		...
		/*Iterator<Cella> iteratore = inputCelle.iterator();
		while(iteratore.hasNext()) {
			Cella cella = iteratore.next();
			risultato.add(cella.getPosizione().getY());
		}*/
		for(Cella cella : inputCelle)
			risultato.add(cella.getPosizione().getY());
		    // equivalente: risultato.add(new Integer (cella.getPosizione().getY()));
		return risultato;
	}

	/**
	 * @param y ordinata della riga controllata
	 * @return true se e solo se la riga Ã¨ attualmente completa
	 */
	...
	Set<Cella> getCelleDellaRigaSenzaBordo(int riga) {
		...
		return celleYX.subSet(bordoSX(riga), false, bordoDX(riga), false);
	}
	...
	*/
	...
	NavigableSet<Cella> getCelleSopraRigaYdecrescente(int riga) {
		...
		NavigableSet<Cella> senzaBordo = celleXY.subSet(fondo(0), false, bordoDX(0), false);
